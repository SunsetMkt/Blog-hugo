import { IOPipeline, DemuxPipeline, AudioDecodePipeline, AudioRenderPipeline, VideoRenderPipeline, Stats } from '@libmedia/avpipeline';
import { Emitter, type EmitterOptions, type Data, type Fn } from '@libmedia/common';
import { type Thread, type WebAssemblyResource, compileResource as compile } from '@libmedia/cheap';
import { AVCodecID, AVMediaType, type AVStreamInterface } from '@libmedia/avutil';
import { RenderMode } from '@libmedia/avrender';
import { type AVFormatContextInterface } from '@libmedia/avformat';
import { FetchIOLoader, FileIOLoader, CustomIOLoader, type WebSocketOptions, WebSocketIOLoader, SocketIOLoader, WebTransportIOLoader, type IOLoaderOptions } from '@libmedia/avnetwork';
...
export default class AVPlayer extends Emitter implements ControllerObserver {
    ...
    getStreams(): {
         ...
         */
        mediaType: string;
        codecparProxy: import("@libmedia/avutil").AVCodecParameters;
        index: int32;
        id: int32;
        codecpar: pointer<import("@libmedia/avutil").AVCodecParameters>;
        nbFrames: int64;
        metadata: Data;
        duration: int64;
        startTime: int64;
        disposition: int32;
        timeBase: import("@libmedia/avutil").AVRational;
        attachedPic: pointer<import("@libmedia/avutil").AVPacket>;
    }[];
    /**
     * 获取当前选择播放的视频流 id
     *
     ...
     * @returns
     */
    getChapters(): import("@libmedia/avformat").AVChapter[];
    /**
     * 获取总时长（毫秒）
     *
     * @returns
     ...
     * @returns
     */
    getVideoList(): Promise<import("@libmedia/avnetwork").IOLoaderVideoStreamInfo>;
    /**
     * 获取音频列表（ dash 和 hls 使用）
     *
     * @returns
     */
    getAudioList(): Promise<import("@libmedia/avnetwork").IOLoaderAudioStreamInfo>;
    /**
     * 获取字幕列表（ dash 和 hls 使用）
     *
     * @returns
     */
    getSubtitleList(): Promise<import("@libmedia/avnetwork").IOLoaderSubtitleStreamInfo>;
    /**
     * 获取 status 状态
     *
     * @returns
